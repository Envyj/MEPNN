...
//
//int main() {
//    cv::VideoCapture cap("./video/有效性测试/1.mp4");
//
//    if (!cap.isOpened()) {
...
//    }
//
//    cv::Mat prevFrame, currFrame, grayPrev, grayCurr, diff;
//
//    cap >> prevFrame;
//    cv::medianBlur(prevFrame, prevFrame, 5);  // Median filtering
//    cv::cvtColor(prevFrame, grayPrev, cv::COLOR_BGR2GRAY);
//
//    int blockSize = std::max(currFrame.cols, currFrame.rows) / 50;  // dynamic block size
//    int overlap = blockSize / 4;  // 25% overlap
//
//    while (true) {
//        cap >> currFrame;
//        if (currFrame.empty()) {
...
//        }
//
//        cv::medianBlur(currFrame, currFrame, 5);  // Median filtering
...
//
//        // Check each block for motion
//        for (int i = 0; i < currFrame.rows - blockSize; i += (blockSize - overlap)) {
//            for (int j = 0; j < currFrame.cols - blockSize; j += (blockSize - overlap)) {
//                cv::Mat block = diff(cv::Rect(j, i, blockSize, blockSize));
//                int motionCount = cv::countNonZero(block);
//                if (motionCount > (blockSize * blockSize) * 0.1) {  // if more than 10% of the block has motion
//                    cv::rectangle(currFrame, cv::Rect(j, i, blockSize, blockSize), cv::Scalar(0, 255, 0), 2);
//                }
//            }
//        }
//
//        // Show the result
//        cv::imshow("Locust-inspired Motion Detection", currFrame);
//        if (cv::waitKey(30) >= 0) {
//            break;
//        }
//
...
////        cv::imshow("Original Frame with Centroids", frame);
...
////
////int main() {
////    saveCentroids("./video/有效性测试/1.mp4");
////    grid_search();
////    cv::destroyAllWindows();
////    return 0;
////}
